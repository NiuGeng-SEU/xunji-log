...
详见 [NOTES.md](NOTES.md) 了解详细特性规格与数学公式设计：

- **体能、疲劳与状态（TSB 模型）**：基于 Banister 冲激响应模型计算滚动的 CTL（体能/长效负荷）、ATL（疲劳/短时疲劳）及 TSB（竞技状态/准备度）指标曲线。
- **力量举三大项（SBD）与相对力量分析**：三大项 PR 追踪、体重相对力量系数（倍体重、Wilks 2.0、DOTS）以及工作组估算 1RM（e1RM）动态演变。
- **智能下次训练预测与推荐**：结合分化恢复周期、阶段化目标与超负荷进阶原则，智能推荐下次训练动作、工作重量、组数与次数（RPE 引导）。
- **移动端适配与 PWA 支持**：专为手机端优化的流式响应布局、触控优化图表及离线 PWA 支持，方便在健身房现场随时查阅与记录。
